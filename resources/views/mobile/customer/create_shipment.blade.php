...
<div class="space-y-8 animate-slide-up pb-32" x-data="bookingForm()">
    ...
    <form action="{{ route('customer.shipments.store') }}" method="POST" class="space-y-10">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-4xl premium-shadow border border-slate-100 dark:border-slate-800 p-8 space-y-4">
                ...
                <input type="text" name="sender_name" value="{{ auth()->user()->name }}" placeholder="Nama Pengirim" class="w-full bg-slate-50 dark:bg-slate-800 border-none h-14 px-6 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-primary-600">
                <input type="tel" name="sender_phone" value="{{ auth()->user()->phone }}" placeholder="No. Telepon" class="w-full bg-slate-50 dark:bg-slate-800 border-none h-14 px-6 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-primary-600">
                <textarea name="sender_address" placeholder="Alamat Lengkap Pengirim" rows="3" class="w-full bg-slate-50 dark:bg-slate-800 border-none p-6 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-primary-600">{{ auth()->user()->address }}</textarea>
            </div>
