...
<div class="space-y-8 animate-slide-up pb-32">
    ...
    <div class="flex items-center justify-between px-1">
        <div class="flex items-center gap-4">
            <a href="{{ auth()->user()->role === 'customer' ? route('customer.dashboard') : route('courier.tasks') }}" class="w-12 h-12 glass rounded-2xl flex items-center justify-center shadow-sm text-indigo-600 tap-scale">
            ...
            <div class="relative z-10 flex flex-col items-center text-center space-y-6">
                ...
                <div class="relative">
                    ...
                    <div class="w-32 h-32 rounded-[2rem] bg-white/10 backdrop-blur-3xl p-1 relative border border-white/20 shadow-2xl overflow-hidden">
                        <img src="{{ $user->photo ? asset('storage/' . $user->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=6366f1&color=fff&bold=true&size=256' }}" 
                ...
                <div class="space-y-1">
                    <h3 class="text-3xl font-black tracking-tight leading-none">{{ $user->name }}</h3>
                    <p class="text-indigo-200/60 text-xs font-bold uppercase tracking-widest">{{ $user->email }}</p>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4 w-full pt-4">
                    <div class="bg-white/5 backdrop-blur-xl rounded-2xl p-3 border border-white/10">
                        ...
                        <p class="text-xs font-bold text-white">{{ ucfirst($user->role) }}</p>
                    </div>
                    <div class="bg-white/5 backdrop-blur-xl rounded-2xl p-3 border border-white/10">
                        <p class="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-1">ID</p>
                        <p class="text-xs font-bold text-white">#{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }}</p>
                    </div>
                    <div class="bg-white/5 backdrop-blur-xl rounded-2xl p-3 border border-white/10">
    ...
    <div id="section-edit-info" class="fixed inset-0 z-[60] bg-slate-950/40 backdrop-blur-md hidden items-end" x-data="{ currentEmail: '{{ $user->email }}' }">
        <div class="w-full bg-white dark:bg-slate-900 rounded-t-[3rem] p-10 pb-12 space-y-8 animate-slide-up shadow-2xl relative">
            ...
            <form id="form-edit-info" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-6" onsubmit="handleProfileUpdate(event)">
                @csrf
                @method('patch')
                ...
                <div class="flex flex-col items-center gap-4 mb-4">
                    <div class="relative group">
                        <div class="w-28 h-28 rounded-[2rem] bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden shadow-inner relative">
                            <img id="photo-preview" src="{{ $user->photo ? asset('storage/' . $user->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=f8fafc&color=6366f1&bold=true' }}" 
                                 class="w-full h-full object-cover {{ $user->photo ? '' : 'opacity-50' }}">
                ...
                <div class="grid grid-cols-1 gap-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="address" rows="2"
                                  class="w-full bg-slate-50 dark:bg-slate-800 p-6 rounded-2xl border-none focus:ring-2 focus:ring-indigo-600 font-bold text-sm transition-all shadow-inner">{{ $user->address }}</textarea>
                    </div>
                </div>
    ...
    <div id="section-change-password" class="fixed inset-0 z-[60] bg-slate-950/40 backdrop-blur-md hidden items-end">
        <div class="w-full bg-white dark:bg-slate-900 rounded-t-[3rem] p-10 pb-12 space-y-8 animate-slide-up shadow-2xl relative">
            ...
            <form id="form-change-password" method="POST" action="{{ route('password.update') }}" class="space-y-6" onsubmit="handlePasswordUpdate(event)">
                @csrf
                @method('put')
                <div class="space-y-5">
                    <div class="space-y-2">
