...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 h-full flex flex-col overflow-hidden">
    ...
    <header class="glass sticky top-0 z-50 px-6 h-20 flex items-center justify-between shrink-0">
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="w-10 h-10 rounded-2xl bg-slate-200 dark:bg-slate-800 overflow-hidden p-0.5 shadow-sm">
                <img src="{{ auth()->user()->photo ? asset('storage/' . auth()->user()->photo) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=6366f1&color=fff&bold=true' }}" 
    ...
    <nav class="glass fixed bottom-6 left-6 right-6 z-50 h-20 rounded-4xl safe-bottom flex items-center justify-around px-4 shadow-2xl">
        <a href="{{ auth()->user()->role === 'customer' ? route('customer.dashboard') : route('courier.tasks') }}" 
           class="flex flex-col items-center justify-center transition-all tap-scale {{ request()->routeIs('*.dashboard') || request()->routeIs('*.tasks') ? 'active-nav' : 'text-slate-400' }}">
        ...
        <a href="{{ route('profile.edit') }}" 
           class="flex flex-col items-center justify-center transition-all tap-scale {{ request()->routeIs('profile.edit') ? 'active-nav' : 'text-slate-400' }}">
    ...
    @if(auth()->user() && auth()->user()->role === 'courier')
        <div id="global-scanner-modal" class="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center p-8 hidden animate-slide-up">
            ...
            <form id="global-claim-form" action="{{ route('courier.shipments.claim') }}" method="POST" class="hidden">
                @csrf
