...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 h-full flex flex-col overflow-hidden">
    ...
    <header class="glass sticky top-0 z-50 px-6 h-20 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest"><?php echo e(auth()->user()->role); ?> Portal</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="w-10 h-10 rounded-2xl bg-slate-200 dark:bg-slate-800 overflow-hidden p-0.5">
                <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(auth()->user()->name)); ?>&background=2563eb&color=fff&bold=true" class="rounded-xl" alt="User">
            </div>
        </div>
    </header>
    ...
    <nav class="glass fixed bottom-6 left-6 right-6 z-50 h-20 rounded-4xl safe-bottom flex items-center justify-around px-4 shadow-2xl">
        <a href="<?php echo e(auth()->user()->role === 'customer' ? route('customer.dashboard') : route('courier.tasks')); ?>" 
           class="flex flex-col items-center justify-center transition-all <?php echo e(request()->routeIs('*.dashboard') || request()->routeIs('*.tasks') ? 'active-nav' : 'text-slate-400'); ?>">
        ...
        <a href="<?php echo e(route('profile.edit')); ?>" 
           class="flex flex-col items-center justify-center transition-all <?php echo e(request()->routeIs('profile.edit') ? 'active-nav' : 'text-slate-400'); ?>">
