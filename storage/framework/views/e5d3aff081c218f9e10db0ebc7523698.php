...
<div class="space-y-8 animate-slide-up">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-4xl premium-shadow border border-slate-100 dark:border-slate-800 p-8 space-y-6">
        <div class="flex justify-between items-start">
            <div class="space-y-1">
                ...
                <h3 class="text-2xl font-black text-primary-600 uppercase"><?php echo e($shipment->tracking_number); ?></h3>
            </div>
        ...
        <div class="flex items-center gap-4 py-6 border-y border-slate-50 dark:border-slate-800">
            <div class="flex-1 text-center space-y-1">
                ...
                <p class="text-sm font-extrabold truncate"><?php echo e($shipment->branch->name); ?></p>
            </div>
            <div class="flex flex-col items-center gap-1">
                ...
                <p class="text-[8px] font-bold text-slate-300 uppercase tracking-tighter"><?php echo e($shipment->service_type); ?></p>
            </div>
            <div class="flex-1 text-center space-y-1">
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tujuan</p>
                <p class="text-sm font-extrabold truncate"><?php echo e(explode(',', $shipment->recipient_address)[0]); ?></p>
            </div>
        </div>
    </div>

    <!-- Payment Section (if pending) -->
    <?php if($shipment->payment_status === 'pending' || $shipment->payment_status === 'unpaid'): ?>
    <div class="bg-primary-600 rounded-4xl p-8 text-white space-y-6 shadow-2xl shadow-primary-200 dark:shadow-none">
        <div class="flex justify-between items-center">
            <div class="space-y-1">
                ...
                <h4 class="text-3xl font-black italic">Rp <?php echo e(number_format($shipment->total_amount, 0, ',', '.')); ?></h4>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="relative space-y-8 pl-4">
            ...
            <?php $__currentLoopData = $shipment->trackings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tracking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="relative flex gap-6">
                    ...
                    <div class="flex-1 space-y-1 pb-4 border-b border-slate-50 dark:border-slate-900/50">
                        <div class="flex justify-between items-start">
                            <h4 class="font-black text-sm text-slate-900 dark:text-slate-100 uppercase tracking-tight"><?php echo e($tracking->status->name); ?></h4>
                            <span class="text-[9px] font-bold text-slate-400"><?php echo e($tracking->created_at->format('H:i • d M')); ?></span>
                        </div>
                        <p class="text-xs text-slate-500 font-medium leading-relaxed"><?php echo e($tracking->location); ?></p>
            ...
            <div class="relative flex gap-6">
                ...
                <div class="flex-1 space-y-1">
                    <div class="flex justify-between items-start">
                        ...
                        <span class="text-[9px] font-bold text-slate-400"><?php echo e($shipment->created_at->format('H:i • d M')); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    if (payButton) {
        payButton.onclick = function() {
            fetch("<?php echo e(route('payments.midtrans.snap-token', $shipment)); ?>", {
                method: 'POST',
                headers: {
