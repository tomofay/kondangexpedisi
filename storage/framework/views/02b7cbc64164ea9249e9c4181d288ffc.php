...
<div class="space-y-8 animate-slide-up pb-32">
    ...
    <div class="flex items-center gap-4">
        <a href="<?php echo e(auth()->user()->role === 'customer' ? route('customer.dashboard') : route('courier.tasks')); ?>" class="w-12 h-12 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center shadow-sm text-slate-400">
    ...
    <div class="bg-primary-600 rounded-4xl p-8 text-white relative overflow-hidden shadow-2xl shadow-primary-200 dark:shadow-none">
        ...
        <div class="relative z-10 flex flex-col items-center text-center space-y-4">
            <div class="w-24 h-24 rounded-3xl bg-white/20 backdrop-blur-xl p-1 relative">
                <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->name)); ?>&background=fff&color=2563eb&bold=true&size=128" class="rounded-2xl w-full h-full object-cover" alt="Avatar">
                <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-emerald-500 border-4 border-primary-600 rounded-full"></div>
            </div>
            <div>
                <h3 class="text-2xl font-black"><?php echo e($user->name); ?></h3>
                <p class="text-primary-100 text-xs font-bold uppercase tracking-widest"><?php echo e($user->email); ?></p>
            </div>
            <div class="px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-[10px] font-black uppercase tracking-widest">
                <?php echo e($user->role); ?> ID: #<?php echo e(str_pad($user->id, 5, '0', STR_PAD_LEFT)); ?>
        ...
        <div class="w-full bg-white dark:bg-slate-900 rounded-t-4xl p-8 space-y-8 animate-slide-up">
            ...
            <form method="POST" action="<?php echo e(route('profile.update')); ?>" class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="<?php echo e($user->name); ?>" class="w-full bg-slate-50 dark:bg-slate-800 border-none h-14 px-6 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-primary-600">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Email</label>
                    <input type="email" name="email" value="<?php echo e($user->email); ?>" class="w-full bg-slate-50 dark:bg-slate-800 border-none h-14 px-6 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-primary-600">
                </div>
